...
class CloudinaryCroppedService {
  ...
  async uploadCroppedImage(id: string, imageData: string, metadata?: {
    pageInfo?: string;
    pageNumber?: number;
    date?: string;
    ...
    try {
      ...
      if (metadata) {
        const contextData = [
          metadata.pageInfo ? `page_info=${encodeURIComponent(metadata.pageInfo)}` : null,
  ...
  async getCroppedImageInfo(id: string): Promise<CloudinaryCroppedImage | null> {
    try {
      const publicId = this.generatePublicId(id);
